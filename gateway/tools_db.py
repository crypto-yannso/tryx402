...
def seed_verified_tools(db_path: str) -> int:
    """Populate the tools table from seed_verified_tools.json on first run.

    Returns the number of active tools after seeding. Idempotent: a
    non-empty catalogue is never re-seeded.
    """
    import json
    import os
    from pathlib import Path
    ...
    conn = get_db(db_path)
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM tools WHERE is_active = 1")
    if cur.fetchone()[0] > 0:
        conn.close()
        return _count_active(db_path)

    candidates = [
        Path(__file__).resolve().parent / "seed_verified_tools.json",
        Path("/app/gateway/seed_verified_tools.json"),
    ]
    seed_file = next((p for p in candidates if p.exists()), None)
    if seed_file:
        try:
            tools = json.loads(seed_file.read_text())
            now = time.time()
            for t in tools:
                cur.execute("""
                    INSERT OR REPLACE INTO tools (id, provider_name, provider_email, slug, origin, endpoint, method, description, price_usd, payout_eur_per_call, is_active, verified_at, last_check_status, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)
                """, (f"tool_{secrets.token_hex(6)}", t["brand"],
                      "x402-network@artaifact.com", t["slug"], t["origin"],
                      t["endpoint"], t["method"], t["summary"], t["price_usd"],
                      round(t["price_usd"] * 0.8, 4), now, t["status_code"], now))
            conn.commit()
        except Exception as e:  # noqa: BLE001
            print(f"Error seeding verified tools: {e}")
    conn.close()
    return _count_active(db_path)


def _count_active(db_path: str) -> int:
    conn = get_db(db_path)
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM tools WHERE is_active = 1")
    n = cur.fetchone()[0]
