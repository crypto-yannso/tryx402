...
def _get_customer_id(request: Request) -> str:
    """Extract customer_id from X-Customer-ID or X-API-Key header.
    
    For anonymous users: X-Customer-ID header (UUID generated by SDK)
    For API key users: X-API-Key header (backward compatible)
    """
    # Try anonymous auth first (no account needed)
    customer_id = request.headers.get("X-Customer-ID", "").strip()
    if customer_id:
        return customer_id
    
    # Fallback to API key auth (backward compatible)
    api_key = request.headers.get("X-API-Key", "").strip()
    if api_key:
        return api_key
    
    raise HTTPException(status_code=401, detail="Missing X-Customer-ID or X-API-Key header")
